  ...
  <section id="project" class="min-h-screen px-4 py-20 sm:px-6 bg-black">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-16 page-transition">
        <span class="inline-flex px-3 py-1 rounded-full text-xs font-semibold mb-4 bg-[#16db65]/15 text-[#16db65] border border-[#16db65]/25">Frontend Development</span>
        <h1 class="mb-6 text-5xl font-bold tracking-tighter text-white md:text-6xl">
          Code <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#058c42] to-[#16db65]">Craftsmanship</span>
        </h1>
        <p class="max-w-2xl text-xl text-gray-500">Web applications built with performance, accessibility, and beautiful interactions in mind.</p>
      </div>

      <!-- Projects Grid -->
      <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <div
          v-for="(project, index) in frontendProjects"
          :key="project.id"
...
const frontendProjects = [
  ...
  },
  {
    ...
    tags: [
      ...
      { name: 'Pinia', type: 'tool' }
    ],
    meta: [
      { text: '8 months', icon: 'fa-clock' },
      { text: 'Team of 5', icon: 'fa-user' }
    ],
...
const tagClass = (type) => ({
  ...
  branding: 'bg-amber-500/15 text-amber-400 border border-amber-500/25',
...
function navigate(dir) {
  const idx = frontendProjects.findIndex(p => p.id === selectedProject.value.id)
  const next = frontendProjects[idx + dir]
